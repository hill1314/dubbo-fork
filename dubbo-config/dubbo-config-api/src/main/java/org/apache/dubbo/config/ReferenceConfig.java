...
public class ReferenceConfig<T> extends ReferenceConfigBase<T> {
    ...
    /**
     * The {@link Protocol} implementation with adaptive functionality,it will be different in different scenarios. A
     * particular {@link Protocol} implementation is determined by the protocol attribute in the {@link URL}. For
     * example:
     *
     * <li>when the url is
     ...
     * layers, and eventually will get a <b>ProtocolSerializationWrapper</b> or <b>ProtocolFilterWrapper</b> or
    ...
    protected void postProcessAfterScopeModelChanged(ScopeModel oldScopeModel, ScopeModel newScopeModel) {
        ...
        protocolSPI = this.getExtensionLoader(Protocol.class)
                .getAdaptiveExtension();
        proxyFactory = this.getExtensionLoader(ProxyFactory.class)
                .getAdaptiveExtension();
    }

    /**
     * Get a string presenting the service names that the Dubbo interface subscribed. If it is a multiple-values, the
     * content will be a comma-delimited String.
     *
     * @return non-null
    ...
    private void logAndCleanup(Throwable t) {
        try {
            ...
            logger.warn(CONFIG_FAILED_DESTROY_INVOKER, "", "",
                    "Unexpected error occurred when destroy invoker of ReferenceConfig(" + url + ").", t);
        }
        ...
        serviceMetadata.getAttributeMap()
                .remove(PROXY_CLASS_REF);

        // Thrown by checkInvokerAvailable().
        if (t.getClass() == IllegalStateException.class && t.getMessage()
                .contains("No provider available for the service")) {
    ...
    private Map<String, String> appendConfig() {
        ...
        if (!ProtocolUtils.isGeneric(generic)) {
            ...
            if (methods.length == 0) {
                logger.warn(CONFIG_NO_METHOD_FOUND, "", "",
        ...
        if (StringUtils.isEmpty(hostToRegistry)) {
            ...
            throw new IllegalArgumentException(
                    "Specified invalid registry ip from property:" + DUBBO_IP_TO_REGISTRY + ", value:"
                            + hostToRegistry);
        }
    ...
    private T createProxy(Map<String, String> referenceParameters) {
        ...
        if (logger.isInfoEnabled()) {
            logger.info("Referred dubbo service: [" + referenceParameters.get(INTERFACE_KEY) + "]."
                    + (ProtocolUtils.isGeneric(referenceParameters.get(GENERIC_KEY)) ? " it's GenericService "
                    + "reference" : " it's not GenericService reference"));
        }
    ...
    private void meshModeHandleUrl(Map<String, String> referenceParameters) {
        ...
        if (StringUtils.isEmpty(podNamespace)) {
            if (StringUtils.isEmpty(System.getenv("POD_NAMESPACE"))) {
                if (logger.isWarnEnabled()) {
                    logger.warn(CONFIG_FAILED_LOAD_ENV_VARIABLE, "", "",
        ...
        String clusterDomain = Optional.ofNullable(System.getenv("CLUSTER_DOMAIN"))
                .orElse(DEFAULT_CLUSTER_DOMAIN);
        ...
        Integer meshPort = Optional.ofNullable(getProviderPort())
                .orElse(DEFAULT_MESH_PORT);
    ...
    private boolean checkMeshConfig(Map<String, String> referenceParameters) {
        ...
        getScopeModel().getConfigManager()
                .getProtocol(TRIPLE)
    ...
    private void aggregateUrlFromRegistry(Map<String, String> referenceParameters) {
        ...
        if (urls.isEmpty() && shouldJvmRefer(referenceParameters)) {
            URL injvmUrl =
                    new URL(LOCAL_PROTOCOL, LOCALHOST_VALUE, 0, interfaceClass.getName()).addParameters(referenceParameters);
    ...
    /**
     * 创建反射 create a reference invoker
     */
    ...
    private void checkInvokerAvailable(long timeout) throws IllegalStateException {
        ...
        if (!available) {
            ...
            IllegalStateException illegalStateException = new IllegalStateException(
                    "Failed to check the status of the service " + interfaceName
            ...
            logger.error(CLUSTER_NO_VALID_PROVIDER, "provider not started", "", "No provider available.",
    ...
    /**
     * This method should be called right after the creation of this class's instance, before any property in other
     * config modules is used. Check each config modules are created properly and override their properties if
     * necessary.
     */
    protected void checkAndUpdateSubConfigs() {
        ...
        if (ProtocolUtils.isGeneric(generic)) {
            if (interfaceClass != null && !interfaceClass.equals(GenericService.class)) {
                logger.warn(CONFIG_PROPERTY_CONFLICT, "", "", String.format(
                        "Found conflicting attributes for interface type: [interfaceClass=%s] and [generic=%s], "
                                + "because the 'generic' attribute has higher priority than 'interfaceClass', so "
                                + "change 'interfaceClass' to '%s'. "
                                + "Note: it will make this reference bean as a candidate bean of type '%s' instead of"
                                + " '%s' when resolving dependency in Spring.", interfaceClass.getName(), generic,
                        GenericService.class.getName(), GenericService.class.getName(), interfaceClass.getName()));
            }
            interfaceClass = GenericService.class;
        } else {
            try {
                if (getInterfaceClassLoader() != null && (interfaceClass == null
                        || interfaceClass.getClassLoader() != getInterfaceClassLoader())) {
    ...
    /**
     * Figure out should refer the service in the same JVM from configurations. The default behavior is true 1. if injvm
     * is specified, then use it 2. then if a url is specified, then assume it's a remote call 3. otherwise, check scope
     * parameter 4. if scope is not specified but the target service is provided in the same JVM, then prefer to make
     * the local call, which is the default behavior
     */
    protected boolean shouldJvmRefer(Map<String, String> map) {
        ...
        if (isInjvm() == null) {
            ...
            if (StringUtils.isNotEmpty(url)) {
                ...
                isJvmRefer = InjvmProtocol.getInjvmProtocol(getScopeModel())
                        .isInjvmRefer(tmpUrl);
            }
        } else {
            isJvmRefer = isInjvm();
    ...
    /**
     * Return if ReferenceConfig has been initialized Note: Cannot use `isInitilized` as it may be treated as a Java
     * Bean property
     *
     * @return initialized
     */
